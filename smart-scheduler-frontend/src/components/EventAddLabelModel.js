...
export default function EventAddLabelModel() {
  ...
  const [title, setTitle] = useState(labelSelected ? labelSelected.label : '');
  ...
  async function handleSubmit(e) {
    ...
    if (labelSelected && labelSelected.id) {
      ...
      addLabel(result.data.data.id, selectedLabel, title);
    }
    clearForm();
    setShowEventAddLabelModelModal(false);
  }
  return (
    <div className="h-screen w-full fixed left-0 top-0 flex justify-center items-center">
      <form
        className="rounded-lg drop-shadow-2xl w-1/4 min-h-1/4 p-4"
        style={{ backgroundColor: 'rgb(38,37,37,1)' }}
      >
        <header className="px-4 py-2 flex justify-between items-center">
          <span className="text-white text-xl">Create new calendar</span>
          <button
            onClick={() => {
              clearForm();
              setShowEventAddLabelModelModal(false);
        ...
        <div className="px-3">
          <div className="grid grid-cols-1/5 items-end">
            <input
              ...
              value={title}
              required
              className="pt-3 border-0 text-white text-normal font-normal pb-2 w-full border-b-2 border-gray-500 focus:outline-none "
        ...
        <footer className="flex justify-end p-3 mr-4 mt-4">
          <button
            type="submit"
