    ...
    <header className="mb-[23px]">
      <div className="flex justify-center ">
        <div className="pt-[9px] pb-[9px] pl-[41px] pr-[41px] size-[16px] text-white shadow-[0_5px_50px_5px_rgba(255,255,255,0.2)] rounded-b-full w-[273px] h-[45px] flex justify-between">
          <button
            className={`${frame === 'day' ? 'text-teal-300' : ''}`}
            onClick={() => changeFrame('day')}
          >
            Day
          </button>
          <button
            className={`${frame === 'week' ? 'text-teal-300' : ''}`}
            onClick={() => changeFrame('week')}
          >
            Week
          </button>
          <button
            className={`${frame === 'month' ? 'text-teal-300' : ''}`}
            onClick={() => changeFrame('month')}
          >
            Month
          </button>
        </div>
      ...
      <div className="flex mt-8 justify-between">
        <div className="flex items-center">
          <form className="ml-[30px] w-[198px] h-[51px] mr-10 size-[72px]">
            <label
              form="default-search"
          ...
          <h2 className="size-[16px] text-white font-bold text-nowrap">
            {currentDayFrame.format('MMMM YYYY')}
          </h2>
        </div>
        <CreateEventButton />
      </div>
