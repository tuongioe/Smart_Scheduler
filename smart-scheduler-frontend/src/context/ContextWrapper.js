...
export default function ContextWrapper(props) {
  ...
  const [showEventAddLabelModel, setShowEventAddLabelModelModal] =
    useState(false);
  ...
  function deleteLabel(id) {
    setLabels(labels.filter((lbl) => lbl.id !== id));
  }
  ...
  const fetchDate = async () => {
    return axiosClient.get(
      `/api/calendar/year/${currentDayFrame.get('year')}/${
        currentDayFrame.get('month') + 1
      }/${currentDayFrame.get('date')}`
    );
  };
